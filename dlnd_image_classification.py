...
def normalize(x):
    ...
    def itemSoftmax(item):
        if len(np.shape(item)) > 1:
            for _, item1 in enumerate(item):
                itemSoftmax(item1)
        else:
            item2 = np.exp(np.subtract(item, np.max(item)))
            item2 = np.dot(item2, 1. / np.sum(item2))
...
def conv2d_maxpool(x_tensor, conv_num_outputs, conv_ksize, conv_strides, pool_ksize, pool_strides):
    ...
    relu_conv = tf.nn.relu(conv2d)
    return tf.nn.max_pool(relu_conv, ksize=[1, pool_ksize[0], pool_ksize[1], 1], strides=[1, pool_strides[0], pool_strides[1], 1], padding='SAME') 
